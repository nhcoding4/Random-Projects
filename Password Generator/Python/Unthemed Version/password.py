...
def gui(error_status: str) -> None:
    ...
    title_frame = Frame(master=root, width=frame_size[0], height=frame_size[1])
    ...
    image_label = Label(master=title_frame, image=img)
    image_label.pack()
    ...
    mid_frame = Frame(master=root, width=mid_size[0], height=mid_size[1])
    ...
    button_frame = Frame(master=mid_frame, width=mid_size[0], height=mid_size[1] / 2)
    button_frame.pack()
...
def add_password_frame(root: tk.Tk, password: str) -> None:
    ...
    pop_up_size: (int, int) = (300, 110)
    pop_up = tk.Toplevel(root)
    pop_up.geometry(f"{pop_up_size[0]}x{pop_up_size[1]}")
    pop_up.title("Save")

    pop_up_frame = tk.Frame(master=pop_up, width=pop_up_size[0], height=pop_up_size[1])
    pop_up_frame.pack(padx=X_PAD, pady=Y_PAD)

    pop_up_label = tk.Label(master=pop_up_frame, text="Enter A Name:")
    pop_up_label.pack()
    ...
    close_button = tk.Button(master=button_frame, text="Close", command=pop_up.destroy)
...
def view_edit_frame(root: tk.Tk) -> None:
    ...
    view_frame_size: (int, int) = (600, 300)
    view_frame = tk.Toplevel(root)
    ...
    label_grid = tk.Frame(master=main_frame, width=view_frame_size[0], height=1)
    label_grid.pack()

    label_name = Label(master=label_grid, text="Name", font=("Arial", 12))
    label_name.grid(row=0, column=0, padx=50)
    label_value = Label(master=label_grid, text="Value", font=("Arial", 12))
    ...
    row: int = 0
    key_elements: List[tk.Entry] = []
    value_elements: List[tk.Entry] = []

    for key, value in json_data.items():
        new_name = Entry(master=data_frame)
        new_name.insert(INSERT, key)
        new_name.grid(row=row, column=0)

        new_value = Entry(master=data_frame)
    ...
    exit_button = Button(master=button_frame, text="Exit", command=view_frame.destroy)
