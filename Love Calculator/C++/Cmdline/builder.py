...
def main() -> None:
    ...
    if not 2 <= len(sys.argv):
        print("Stupid input dected. Aborting.")
        return
...
def build() -> None:
    """Attempts to build a C++ project using source files found in a /src/ directory."""

    # Get all filesnames in the src directory and save them to a list.
    ...
    # Build a valid command and pass it to the operating system to executue.
    ...
    # Look for presistent options in a textfile.
    ...
    if not os.path.exists(path):
        ...
        with open("options.txt", "r") as file:
            ...
            additonal_options: str = "".join(f" {arg}" for arg in text_options)
            command += additonal_options
