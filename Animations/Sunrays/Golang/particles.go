...
func (p *Particle) pushParticle() {
	dx := float64(p.x - p.mouse.x)
	dy := float64(p.y - p.mouse.y)

	distance := math.Hypot(dx, dy)
	if distance < float64(p.mouse.radius) {
		power := (float64(p.mouse.radius) / distance) * p.powerPushMultiplier
